...
def getNext(user):
    ...
    image_querystring = {"pageNumber":"1","pageSize":"1","q":item.foodName+" food","autoCorrect":"true"}
    image_response = requests.request("GET", image_url, headers=image_headers, params=image_querystring)
    image_response = image_response.json()
    url = image_response["value"][0]["url"]
